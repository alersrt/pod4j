...
public class AuthConfig {
    ...
    /**
     * Email is an optional value associated with the username. This field is deprecated and will be
     * removed in a later version of docker.
     *
     * @return email
     */
    @javax.annotation.Nullable
    ...
    public int hashCode() {
        return Objects.hash(
                auth, email, identitytoken, password, registrytoken, serveraddress, username);
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!AuthConfig.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in AuthConfig is not found in the empty"
                                        + " JSON string",
                                AuthConfig.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `AuthConfig` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
        if ((jsonObj.get("auth") != null && !jsonObj.get("auth").isJsonNull())
                && !jsonObj.get("auth").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `auth` to be a primitive type in the JSON string"
                                    + " but got `%s`",
                            jsonObj.get("auth").toString()));
        }
        ...
        if ((jsonObj.get("registrytoken") != null && !jsonObj.get("registrytoken").isJsonNull())
                && !jsonObj.get("registrytoken").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `registrytoken` to be a primitive type in the JSON"
                                    + " string but got `%s`",
                            jsonObj.get("registrytoken").toString()));
        }
        if ((jsonObj.get("serveraddress") != null && !jsonObj.get("serveraddress").isJsonNull())
                && !jsonObj.get("serveraddress").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `serveraddress` to be a primitive type in the JSON"
                                    + " string but got `%s`",
                            jsonObj.get("serveraddress").toString()));
        }
