...
public class EndpointIPAMConfig {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!EndpointIPAMConfig.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in EndpointIPAMConfig is not found in the"
                                        + " empty JSON string",
                                EndpointIPAMConfig.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `EndpointIPAMConfig` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
        if ((jsonObj.get("IPv4Address") != null && !jsonObj.get("IPv4Address").isJsonNull())
                && !jsonObj.get("IPv4Address").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `IPv4Address` to be a primitive type in the JSON"
                                    + " string but got `%s`",
                            jsonObj.get("IPv4Address").toString()));
        }
        if ((jsonObj.get("IPv6Address") != null && !jsonObj.get("IPv6Address").isJsonNull())
                && !jsonObj.get("IPv6Address").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `IPv6Address` to be a primitive type in the JSON"
                                    + " string but got `%s`",
                            jsonObj.get("IPv6Address").toString()));
        }
        // ensure the optional json data is an array if present
        if (jsonObj.get("LinkLocalIPs") != null
                && !jsonObj.get("LinkLocalIPs").isJsonNull()
                && !jsonObj.get("LinkLocalIPs").isJsonArray()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `LinkLocalIPs` to be an array in the JSON string"
                                    + " but got `%s`",
                            jsonObj.get("LinkLocalIPs").toString()));
        }
    }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<EndpointIPAMConfig>() {
                        @Override
                        public void write(JsonWriter out, EndpointIPAMConfig value)
                                throws IOException {
