...
/**
 * IDMap contains a single entry for user namespace range remapping. An array of IDMap entries
 * represents the structure that will be provided to the Linux kernel for creating a user namespace.
 */
...
public class IDMap {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!IDMap.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in IDMap is not found in the empty JSON"
                                        + " string",
                                IDMap.openapiRequiredFields.toString()));
            }
        }
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!IDMap.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the `IDMap`"
                                        + " properties. JSON: %s",
