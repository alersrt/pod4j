...
public class ImageDelete200ResponseInner {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!ImageDelete200ResponseInner.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in ImageDelete200ResponseInner is not"
                                        + " found in the empty JSON string",
                                ImageDelete200ResponseInner.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `ImageDelete200ResponseInner` properties. JSON: %s",
        ...
        if ((jsonObj.get("deleted") != null && !jsonObj.get("deleted").isJsonNull())
            ...
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `deleted` to be a primitive type in the JSON string"
                                    + " but got `%s`",
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!ImageDelete200ResponseInner.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'ImageDelete200ResponseInner' and its
                // subtypes
            }
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<ImageDelete200ResponseInner>() {
                        @Override
                        public void write(JsonWriter out, ImageDelete200ResponseInner value)
                                throws IOException {
