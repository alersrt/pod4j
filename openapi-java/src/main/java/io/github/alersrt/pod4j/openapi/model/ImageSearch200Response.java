...
public class ImageSearch200Response {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!ImageSearch200Response.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in ImageSearch200Response is not found in"
                                        + " the empty JSON string",
                                ImageSearch200Response.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `ImageSearch200Response` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
        if ((jsonObj.get("Automated") != null && !jsonObj.get("Automated").isJsonNull())
                && !jsonObj.get("Automated").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `Automated` to be a primitive type in the JSON"
                                    + " string but got `%s`",
                            jsonObj.get("Automated").toString()));
        }
        ...
        if ((jsonObj.get("Official") != null && !jsonObj.get("Official").isJsonNull())
                && !jsonObj.get("Official").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `Official` to be a primitive type in the JSON"
                                    + " string but got `%s`",
                            jsonObj.get("Official").toString()));
        }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<ImageSearch200Response>() {
                        @Override
                        public void write(JsonWriter out, ImageSearch200Response value)
                                throws IOException {
