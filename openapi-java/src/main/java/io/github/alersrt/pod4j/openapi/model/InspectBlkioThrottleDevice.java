...
/**
 * InspectBlkioThrottleDevice holds information about a speed cap for a device node. This cap
 * applies to a specific operation (read, write, etc) on the given node.
 */
...
public class InspectBlkioThrottleDevice {
    ...
    /**
     * Rate is the maximum rate. It is in either bytes per second or iops per second, determined by
     * where it is used - documentation will indicate which is appropriate.
     *
     * @return rate
     */
    @javax.annotation.Nullable
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!InspectBlkioThrottleDevice.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in InspectBlkioThrottleDevice is not"
                                        + " found in the empty JSON string",
                                InspectBlkioThrottleDevice.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `InspectBlkioThrottleDevice` properties. JSON: %s",
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!InspectBlkioThrottleDevice.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'InspectBlkioThrottleDevice' and its
                // subtypes
            }
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<InspectBlkioThrottleDevice>() {
                        @Override
                        public void write(JsonWriter out, InspectBlkioThrottleDevice value)
                                throws IOException {
