...
public class InspectIDMappings {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!InspectIDMappings.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in InspectIDMappings is not found in the"
                                        + " empty JSON string",
                                InspectIDMappings.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `InspectIDMappings` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
        // ensure the optional json data is an array if present
        if (jsonObj.get("GidMap") != null
                && !jsonObj.get("GidMap").isJsonNull()
                && !jsonObj.get("GidMap").isJsonArray()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `GidMap` to be an array in the JSON string but got"
                                    + " `%s`",
                            jsonObj.get("GidMap").toString()));
        }
        // ensure the optional json data is an array if present
        if (jsonObj.get("UidMap") != null
                && !jsonObj.get("UidMap").isJsonNull()
                && !jsonObj.get("UidMap").isJsonArray()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `UidMap` to be an array in the JSON string but got"
                                    + " `%s`",
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<InspectIDMappings>() {
                        @Override
                        public void write(JsonWriter out, InspectIDMappings value)
                                throws IOException {
