...
public class LeaseRange {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!LeaseRange.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in LeaseRange is not found in the empty"
                                        + " JSON string",
                                LeaseRange.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `LeaseRange` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
        if ((jsonObj.get("end_ip") != null && !jsonObj.get("end_ip").isJsonNull())
                && !jsonObj.get("end_ip").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `end_ip` to be a primitive type in the JSON string"
                                    + " but got `%s`",
                            jsonObj.get("end_ip").toString()));
        }
        if ((jsonObj.get("start_ip") != null && !jsonObj.get("start_ip").isJsonNull())
                && !jsonObj.get("start_ip").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `start_ip` to be a primitive type in the JSON"
                                    + " string but got `%s`",
                            jsonObj.get("start_ip").toString()));
        }
    }
