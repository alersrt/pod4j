 ...
 */
@JsonPropertyOrder({
  LinuxPersonality.JSON_PROPERTY_DOMAIN,
  LinuxPersonality.JSON_PROPERTY_FLAGS
})
...
public class LinuxPersonality {
  ...
  /**
   * Return true if this LinuxPersonality object is equal to o.
   */
  @Override
    ...
    if (getFlags() != null) {
      for (int i = 0; i < getFlags().size(); i++) {
        joiner.add(String.format("%sflags%s%s=%s", prefix, suffix,
            ...
            URLEncoder.encode(ApiClient.valueToString(getFlags().get(i)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
      }
    }
