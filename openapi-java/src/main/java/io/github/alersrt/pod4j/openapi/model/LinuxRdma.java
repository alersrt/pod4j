...
public class LinuxRdma {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!LinuxRdma.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(String.format("The required field(s) %s in LinuxRdma is not found in the empty JSON string", LinuxRdma.openapiRequiredFields));
            }
        }
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!LinuxRdma.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `LinuxRdma` properties. JSON: %s", entry.getKey(), jsonElement));
            }
        }
    ...
    public String toString() {
        String sb = "class LinuxRdma {\n" +
                "    hcaHandles: " + toIndentedString(hcaHandles) + "\n" +
                "    hcaObjects: " + toIndentedString(hcaObjects) + "\n" +
                "}";
        return sb;
    }
