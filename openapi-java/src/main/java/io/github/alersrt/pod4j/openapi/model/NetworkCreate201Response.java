...
public class NetworkCreate201Response {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!NetworkCreate201Response.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in NetworkCreate201Response is not found"
                                        + " in the empty JSON string",
                                NetworkCreate201Response.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `NetworkCreate201Response` properties. JSON: %s",
        ...
        if ((jsonObj.get("Warning") != null && !jsonObj.get("Warning").isJsonNull())
                && !jsonObj.get("Warning").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `Warning` to be a primitive type in the JSON string"
                                    + " but got `%s`",
                            jsonObj.get("Warning").toString()));
        }
    }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!NetworkCreate201Response.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'NetworkCreate201Response' and its
                // subtypes
            }
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<NetworkCreate201Response>() {
                        @Override
                        public void write(JsonWriter out, NetworkCreate201Response value)
                                throws IOException {
