...
public class NetworkPrune200Response {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!NetworkPrune200Response.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in NetworkPrune200Response is not found"
                                        + " in the empty JSON string",
                                NetworkPrune200Response.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `NetworkPrune200Response` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
        // ensure the optional json data is an array if present
        if (jsonObj.get("NetworksDeleted") != null
                && !jsonObj.get("NetworksDeleted").isJsonNull()
                && !jsonObj.get("NetworksDeleted").isJsonArray()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `NetworksDeleted` to be an array in the JSON string"
                                    + " but got `%s`",
                            jsonObj.get("NetworksDeleted").toString()));
        }
    }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!NetworkPrune200Response.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'NetworkPrune200Response' and its
                // subtypes
            }
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<NetworkPrune200Response>() {
                        @Override
                        public void write(JsonWriter out, NetworkPrune200Response value)
                                throws IOException {
