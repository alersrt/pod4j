...
public class NetworkUpdateOptions {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!NetworkUpdateOptions.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in NetworkUpdateOptions is not found in"
                                        + " the empty JSON string",
                                NetworkUpdateOptions.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `NetworkUpdateOptions` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
        // ensure the optional json data is an array if present
        if (jsonObj.get("adddnsservers") != null
                && !jsonObj.get("adddnsservers").isJsonNull()
                && !jsonObj.get("adddnsservers").isJsonArray()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `adddnsservers` to be an array in the JSON string"
                                    + " but got `%s`",
                            jsonObj.get("adddnsservers").toString()));
        }
        // ensure the optional json data is an array if present
        if (jsonObj.get("removednsservers") != null
                && !jsonObj.get("removednsservers").isJsonNull()
                && !jsonObj.get("removednsservers").isJsonArray()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `removednsservers` to be an array in the JSON"
                                    + " string but got `%s`",
                            jsonObj.get("removednsservers").toString()));
        }
    }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<NetworkUpdateOptions>() {
                        @Override
                        public void write(JsonWriter out, NetworkUpdateOptions value)
                                throws IOException {
