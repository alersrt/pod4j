    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!PodTopOKBody.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(String.format("The required field(s) %s in PodTopOKBody is not found in the empty JSON string", PodTopOKBody.openapiRequiredFields));
            }
        }
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!PodTopOKBody.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PodTopOKBody` properties. JSON: %s", entry.getKey(), jsonElement));
            }
        }
