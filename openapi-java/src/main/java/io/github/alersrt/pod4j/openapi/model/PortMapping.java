...
public class PortMapping {
    ...
    /**
     * HostIP is the IP that we will bind to on the host. If unset, assumed to be 0.0.0.0 (all
     * interfaces).
     *
     * @return hostIp
     */
    ...
    /**
     * HostPort is the port number that will be forwarded from the host into the container. If
     * omitted, a random port on the host (guaranteed to be over 1024) will be assigned.
     *
     * @return hostPort
     */
    @javax.annotation.Nullable
    ...
    /**
     * Protocol is the protocol forward. Must be either \&quot;tcp\&quot;, \&quot;udp\&quot;, and
     * \&quot;sctp\&quot;, or some combination of these separated by commas. If unset, assumed to be
     * TCP.
     *
     * @return protocol
     */
    ...
    /**
     * Range is the number of ports that will be forwarded, starting at HostPort and ContainerPort
     * and counting up. This is 1-indexed, so 1 is assumed to be a single port (only the
     * Hostport:Containerport mapping will be added), 2 is two ports (both Hostport:Containerport
     * and Hostport+1:Containerport+1), etc. If unset, assumed to be 1 (a single port). Both
     * hostport + range and containerport + range must be less than 65536.
     *
     * @return range
     */
    @javax.annotation.Nullable
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!PortMapping.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in PortMapping is not found in the empty"
                                        + " JSON string",
                                PortMapping.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `PortMapping` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
        if ((jsonObj.get("host_ip") != null && !jsonObj.get("host_ip").isJsonNull())
                && !jsonObj.get("host_ip").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `host_ip` to be a primitive type in the JSON string"
                                    + " but got `%s`",
                            jsonObj.get("host_ip").toString()));
        }
