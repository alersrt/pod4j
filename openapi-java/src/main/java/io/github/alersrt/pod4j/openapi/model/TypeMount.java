...
public class TypeMount {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!TypeMount.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in TypeMount is not found in the empty"
                                        + " JSON string",
                                TypeMount.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `TypeMount` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
        if ((jsonObj.get("FsType") != null && !jsonObj.get("FsType").isJsonNull())
                && !jsonObj.get("FsType").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `FsType` to be a primitive type in the JSON string"
                                    + " but got `%s`",
                            jsonObj.get("FsType").toString()));
        }
        // ensure the optional json data is an array if present
        if (jsonObj.get("MountFlags") != null
                && !jsonObj.get("MountFlags").isJsonNull()
                && !jsonObj.get("MountFlags").isJsonArray()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `MountFlags` to be an array in the JSON string but"
                                    + " got `%s`",
                            jsonObj.get("MountFlags").toString()));
        }
    }
