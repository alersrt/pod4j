    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!WaitExitError.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(String.format("The required field(s) %s in WaitExitError is not found in the empty JSON string", WaitExitError.openapiRequiredFields));
            }
        }
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!WaitExitError.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `WaitExitError` properties. JSON: %s", entry.getKey(), jsonElement));
            }
        }
